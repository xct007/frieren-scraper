...
async function search (query: string): Promise<YoutubeSearchResult[] | errorHandling> {
	try {
		...
		if (_initData.sectionListRenderer) {
			_render = _initData.sectionListRenderer.contents.filter((item: any) => item?.itemSectionRenderer?.contents.filter((v: any) => v.videoRenderer || v.playlistRenderer || v.channelRenderer)
			).shift().itemSectionRenderer.contents
		}
		if (_initData.richGridRenderer) {
			_render = _initData.richGridRenderer.contents.filter((item: any) =>
		...
		for (const item of _render) {
			if (item.videoRenderer && item.videoRenderer.lengthText) {
				...
				const thumbnail: string = video?.thumbnail?.thumbnails[video?.thumbnail?.thumbnails.length - 1].url || ''
				const uploaded: string = video?.publishedTimeText?.simpleText || ''
				const views: string = video?.viewCountText?.simpleText?.replace(/[^0-9.]/g, '') || ''
