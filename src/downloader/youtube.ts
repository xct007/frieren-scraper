...
async function search(
	...
	try {
		...
		const _initData =
			JSON.parse(_string).contents.twoColumnSearchResultsRenderer.primaryContents;
			...
			_render = _initData.sectionListRenderer.contents
				.filter((item: any) =>
					item?.itemSectionRenderer?.contents.filter(
						(v: any) => v.videoRenderer || v.playlistRenderer || v.channelRenderer
					)
				)
		...
		if (_initData.richGridRenderer) {
			_render = _initData.richGridRenderer.contents
				.filter((item: any) => item.richGridRenderer && item.richGridRenderer.contents)
