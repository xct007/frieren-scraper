		...
		$("div[role='tabpanel'] > div").each((i: number, e: Element) => {
			...
			$(_metadata).each((_i: number, _e: Element) => {
				const _sum_title: string = $(_e)
					...
					.trim()
					.toLowerCase();
				const _sum_content: string = $(_e).find(".summary-content").text().trim();
				_metadatas.push({
...
export async function detail(
	...
	try {
		...
		const $ = Cheerio(data);
		const title: string =
			$(".post-title > h1").text().trim() || $("#chapter-heading.h4").text().trim();
		...
		if (url.includes("episode-")) {
			const _iframe_source_url: string = $(".player_logic_item > iframe").attr("src");
		...
		$(".listing-chapters_wrap > ul > li").each((i: number, e: Element) => {
			...
			const release_date: string = $(e).find(".chapter-release-date > i").text();
			const url: string = $(e).find("a").attr("href");
			episodes.push({
				episode,
				release_date,
