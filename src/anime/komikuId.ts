...
class komiku {
	...
	public static async latest(): Promise<KomikuIdLatestResults[] | errorHandling> {
		try {
			...
			$(komiku._latestLoader.base).each((i: number, e: Element) => {
				const title: string = $(e).find(komiku._latestLoader.titleLoader).text();
				const updated: string = $(e).find(komiku._latestLoader.updatedLoader).text();
				const chapter: string = $(e).find(komiku._latestLoader.chapterLoader).text();
				let url: string = $(e)
	...
	public static async detail(
		...
		KomikuIdDetailResult["Manga"] | KomikuIdDetailResult["Chapter"] | errorHandling
	> {
		try {
			...
			if (/\/manga\//i.test(url)) {
				const title: string = $(komiku._detailLoader.manga.titleLoader.title)
					.text()
					.trim();
				const description: string = $(komiku._detailLoader.manga.descriptionLoader)
					.text()
					.trim();
				...
				const images: string[] = [];
				$(komiku._detailLoader.chapter.imagesLoader.base).each((i: any, e: Element) => {
